gsea_gomf<-function(h,...){
	...
	gsea_goMF_params<-function(h,h2,h3,h4,h5,h6,...){

		run_params<-function(h,h2,h3,h4,h5,h6,h7,h8,...){
			ts_name=h;ts_val=h2;ann_pk<-h3;db<-h4;stat_sig<-h5;p_v<-h6;children=h7;r_choice=h8;
pb_ma<-tkProgressBar(title="GSEA GO Molecular Function...",label="",min=0,max=1,initial=0.1,width=500)
...
setTkProgressBar(pb_ma,0.9,title=NULL,label="")
			...
			for(i in 0:dim(res2)[1]){ for(j in 0:dim(res2)[2]){ 
				if(i==0){ gsea_gomf[[i,j]]<-colnames(res2)[j] } else { 
				if(j==0){ gsea_gomf[[i,j]]<-rownames(res2)[i] } else {
				tem<-res2[i,j]
				gsea_gomf[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
				} }
			} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
			if(ts_name=="Affymetrix"){
				GOresultMF_Affy[[ts_val]]<<-res2
				ta_affy_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			}
			if(ts_name=="Agilent-OneColor"){
				GOresultMF_Ag1[[ts_val]]<<-res2
				ta_ag1_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Agilent-TwoColor"){
				GOresultMF_Ag2[[ts_val]]<<-res2
				ta_ag2_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Illumina-Beadarray"){
				GOresultMF_Il_B[[ts_val]]<<-res2
				ta_ilb_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Illumina-Lumi"){
				GOresultMF_Il_L[[ts_val]]<<-res2
				ta_ill_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Nimblegen"){
				GOresultMF_N[[ts_val]]<<-res2
				ta_nbl_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Series-Matrix"){
				GOresultMF_S[[ts_val]]<<-res2
				ta_smt_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
			...
			if(ts_name=="Online-Data"){
				GOresultMF_O[[ts_val]]<<-res2
				ta_onl_gsea_gomf[[ts_val]]<<-gsea_gomf
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOMF"))}
				}
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			gsea_goMF_params(ts_name,ts_val,pre_rs,p_value_s,children,r_choice)
		})
	})
}
