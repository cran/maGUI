graph_gsea_goCC<-function(h,...){
	...
	try(({
		...
		if(exists("GOresultCC_N"))ff=length(GOresultCC_N)
	...
	legend_gsea_goCC_N=NULL;legend_gsea_goCC_S=NULL;legend_gsea_goCC_O=NULL;p_v=NULL;view_ww=NULL;
	...
	legend_gsea_goCC_N,legend_gsea_goCC_S,legend_gsea_goCC_O,p_v,view_ww)
		...
		y2<-gbutton("OK",border=TRUE,handler=function(h,...){
			if(length(x)!=0){
				...
				if(length(which(x=="Affymetrix"))!=0)
				{
					...
					GO.vec<-rownames(GOresultCC_Affy)[GOresultCC_Affy$Pvalue<=p_v]
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_Ag1)[GOresultCC_Ag1$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_Ag2)[GOresultCC_Ag2$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_Il_B)[GOresultCC_Il_B$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_Il_L)[GOresultCC_Il_L$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_N)[GOresultCC_N$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_S)[GOresultCC_S$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
				...
				{
					try(dispose(view_ww),silent=TRUE)
					GO.vec<-rownames(GOresultCC_O)[GOresultCC_O$Pvalue<=p_v]
					g<-GOGraph(GO.vec,GOCCPARENTS)
					g<-removeNode("all",g)
					nodes<-buildNodeList(g)
					focusnode<-sapply(nodes,name) %in% GO.vec
