graph_gsta_gobp<-function(h,...){
	...
	graph_gsta_goBP_params<-function(h,h2,h3,h4,h5,h6,h7,...){
		ts_name=h;ts_val=h2;gobp<-h3;p_v_1<-h4;p_v_2<-h5;children=h6;r_choice=h7;
pb_ma<-tkProgressBar(title="Graph GSTA GO Biological Process...",label="",min=0,max=1,initial=0.1,width=500)
setTkProgressBar(pb_ma,0.1,title=NULL,label="")
		p1<-rownames(gobp)[gobp$'p-value'>=p_v_1]
		p2<-rownames(gobp)[gobp$'p-value'<=p_v_2]
		p<-p2[match(p1,p2)]
...
setTkProgressBar(pb_ma,0.5,title=NULL,label="")
		...
		graph_gsta_goBP<-plot(g,nodeAttrs=nAttrs)
setTkProgressBar(pb_ma,0.8,title=NULL,label="")
		terms<-getGOTerm(nodes(g))
		legend<-data.frame(terms)
		g_colrs<-data.frame(nodefill)
		legend_gsta_goBP<-data.frame(g_colrs,legend)
		colnames(legend_gsta_goBP)<-c("Node_Color","BP")
setTkProgressBar(pb_ma,0.9,title=NULL,label="")
		ta_legend_gsta_gobp<-tclArray()
		for(i in 0:dim(legend_gsta_goBP)[1]){ for(j in 0:dim(legend_gsta_goBP)[2]){ 
			if(i==0){ ta_legend_gsta_gobp[[i,j]]<-colnames(legend_gsta_goBP)[j] } else { 
			if(j==0){ ta_legend_gsta_gobp[[i,j]]<-rownames(legend_gsta_goBP)[i] } else {
			tem<-legend_gsta_goBP[i,j]
			ta_legend_gsta_gobp[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
			} }
		} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
		if(ts_name=="Affymetrix"){
			graph_gsta_goBP_Affy[[ts_val]]<<-g
			nodefill_gsta_goBP_Affy[[ts_val]]<<-nodefill
			legend_gsta_goBP_Affy[[ts_val]]<<-legend_gsta_goBP
			ta_affy_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_Affy[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_Affy[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
			}
		...
		if(ts_name=="Agilent-OneColor"){
			graph_gsta_goBP_Ag1[[ts_val]]<<-g
			nodefill_gsta_goBP_Ag1[[ts_val]]<<-nodefill
			legend_gsta_goBP_Ag1[[ts_val]]<<-legend_gsta_goBP
			ta_ag1_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_Ag1[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_Ag1[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Agilent-TwoColor"){
			graph_gsta_goBP_Ag2[[ts_val]]<<-g
			nodefill_gsta_goBP_Ag2[[ts_val]]<<-nodefill
			legend_gsta_goBP_Ag2[[ts_val]]<<-legend_gsta_goBP
			ta_ag2_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_Ag2[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_Ag2[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Illumina-Beadarray"){
			graph_gsta_goBP_Il_B[[ts_val]]<<-g
			nodefill_gsta_goBP_Il_B[[ts_val]]<<-nodefill
			legend_gsta_goBP_Il_B[[ts_val]]<<-legend_gsta_goBP
			ta_ilb_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_Il_B[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_Il_B[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Illumina-Lumi"){
			graph_gsta_goBP_Il_L[[ts_val]]<<-g
			nodefill_gsta_goBP_Il_L[[ts_val]]<<-nodefill
			legend_gsta_goBP_Il_L[[ts_val]]<<-legend_gsta_goBP
			ta_ill_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_Il_L[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_Il_L[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Nimblegen"){
			graph_gsta_goBP_N[[ts_val]]<<-g
			nodefill_gsta_goBP_N[[ts_val]]<<-nodefill
			legend_gsta_goBP_N[[ts_val]]<<-legend_gsta_goBP
			ta_nbl_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_N[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_N[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Series-Matrix"){
			graph_gsta_goBP_S[[ts_val]]<<-g
			nodefill_gsta_goBP_S[[ts_val]]<<-nodefill
			legend_gsta_goBP_S[[ts_val]]<<-legend_gsta_goBP
			ta_smt_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_S[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_S[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
		...
		if(ts_name=="Online-Data"){
			graph_gsta_goBP_O[[ts_val]]<<-g
			nodefill_gsta_goBP_O[[ts_val]]<<-nodefill
			legend_gsta_goBP_O[[ts_val]]<<-legend_gsta_goBP
			ta_onl_legend_gsta_gobp[[ts_val]]<<-ta_legend_gsta_gobp
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goBP_O[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goBP_O[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOBP_Graph"))
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			if(ts_name=="Affymetrix"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_Affy[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Agilent-OneColor"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_Ag1[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Agilent-TwoColor"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_Ag2[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Illumina-Beadarray"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_Il_B[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Illumina-Lumi"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_Il_L[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Nimblegen"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_N[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Series-Matrix"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_S[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Online-Data"){
				graph_gsta_goBP_params(ts_name,ts_val,GOtable.outBP_O[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
		})
	})
}
