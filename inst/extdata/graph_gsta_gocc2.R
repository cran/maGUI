graph_gsta_gocc<-function(h,...){
	...
	graph_gsta_goCC_params<-function(h,h2,h3,h4,h5,h6,h7,...){
		ts_name=h;ts_val=h2;gocc<-h3;p_v_1<-h4;p_v_2<-h5;children=h6;r_choice=h7;
pb_ma<-tkProgressBar(title="Graph GSTA GO Cellular Component...",label="",min=0,max=1,initial=0.1,width=500)
setTkProgressBar(pb_ma,0.1,title=NULL,label="")
		p1<-rownames(gocc)[gocc$'p-value'>=p_v_1]
		p2<-rownames(gocc)[gocc$'p-value'<=p_v_2]
		p<-p2[match(p1,p2)]
...
setTkProgressBar(pb_ma,0.5,title=NULL,label="")
		...
		graph_gsta_goCC<-plot(g,nodeAttrs=nAttrs)
setTkProgressBar(pb_ma,0.8,title=NULL,label="")
		terms<-getGOTerm(nodes(g))
		legend<-data.frame(terms)
		g_colrs<-data.frame(nodefill)
		legend_gsta_goCC<-data.frame(g_colrs,legend)
		colnames(legend_gsta_goCC)<-c("Node_Color","CC")
setTkProgressBar(pb_ma,0.9,title=NULL,label="")
		ta_legend_gsta_gocc<-tclArray()
		for(i in 0:dim(legend_gsta_goCC)[1]){ for(j in 0:dim(legend_gsta_goCC)[2]){ 
			if(i==0){ ta_legend_gsta_gocc[[i,j]]<-colnames(legend_gsta_goCC)[j] } else { 
			if(j==0){ ta_legend_gsta_gocc[[i,j]]<-rownames(legend_gsta_goCC)[i] } else {
			tem<-legend_gsta_goCC[i,j]
			ta_legend_gsta_gocc[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
			} }
		} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
		if(ts_name=="Affymetrix"){
			graph_gsta_goCC_Affy[[ts_val]]<<-g
			nodefill_gsta_goCC_Affy[[ts_val]]<<-nodefill
			legend_gsta_goCC_Affy[[ts_val]]<<-legend_gsta_goCC
			ta_affy_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_Affy[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_Affy[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
			}
		...
		if(ts_name=="Agilent-OneColor"){
			graph_gsta_goCC_Ag1[[ts_val]]<<-g
			nodefill_gsta_goCC_Ag1[[ts_val]]<<-nodefill
			legend_gsta_goCC_Ag1[[ts_val]]<<-legend_gsta_goCC
			ta_ag1_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_Ag1[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_Ag1[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Agilent-TwoColor"){
			graph_gsta_goCC_Ag2[[ts_val]]<<-g
			nodefill_gsta_goCC_Ag2[[ts_val]]<<-nodefill
			legend_gsta_goCC_Ag2[[ts_val]]<<-legend_gsta_goCC
			ta_ag2_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_Ag2[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_Ag2[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Illumina-Beadarray"){
			graph_gsta_goCC_Il_B[[ts_val]]<<-g
			nodefill_gsta_goCC_Il_B[[ts_val]]<<-nodefill
			legend_gsta_goCC_Il_B[[ts_val]]<<-legend_gsta_goCC
			ta_ilb_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_Il_B[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_Il_B[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Illumina-Lumi"){
			graph_gsta_goCC_Il_L[[ts_val]]<<-g
			nodefill_gsta_goCC_Il_L[[ts_val]]<<-nodefill
			legend_gsta_goCC_Il_L[[ts_val]]<<-legend_gsta_goCC
			ta_ill_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_Il_L[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_Il_L[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Nimblegen"){
			graph_gsta_goCC_N[[ts_val]]<<-g
			nodefill_gsta_goCC_N[[ts_val]]<<-nodefill
			legend_gsta_goCC_N[[ts_val]]<<-legend_gsta_goCC
			ta_nbl_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_N[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_N[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Series-Matrix"){
			graph_gsta_goCC_S[[ts_val]]<<-g
			nodefill_gsta_goCC_S[[ts_val]]<<-nodefill
			legend_gsta_goCC_S[[ts_val]]<<-legend_gsta_goCC
			ta_smt_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_S[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_S[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
		...
		if(ts_name=="Online-Data"){
			graph_gsta_goCC_O[[ts_val]]<<-g
			nodefill_gsta_goCC_O[[ts_val]]<<-nodefill
			legend_gsta_goCC_O[[ts_val]]<<-legend_gsta_goCC
			ta_onl_legend_gsta_gocc[[ts_val]]<<-ta_legend_gsta_gocc
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_goCC_O[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_goCC_O[[ts_val]],nodeAttrs=nAttrs)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_GOCC_Graph"))
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			if(ts_name=="Affymetrix"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_Affy[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Agilent-OneColor"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_Ag1[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Agilent-TwoColor"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_Ag2[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Illumina-Beadarray"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_Il_B[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Illumina-Lumi"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_Il_L[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Nimblegen"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_N[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Series-Matrix"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_S[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
			if(ts_name=="Online-Data"){
				graph_gsta_goCC_params(ts_name,ts_val,GOtable.outCC_O[[ts_val]],p_v_1,p_v_2,children,r_choice)
			}
		})
	})
}
