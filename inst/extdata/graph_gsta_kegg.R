graph_gsta_kegg<-function(h,...){
	try(({KEGGtable.out_Affy<-KEGGtable.out_Affy;KEGGtable.out_Ag1<-KEGGtable.out_Ag1;KEGGtable.out_Ag2<-KEGGtable.out_Ag2;
		KEGGtable.out_Il_B<-KEGGtable.out_Il_B;KEGGtable.out_Il_L<-KEGGtable.out_Il_L;KEGGtable.out_N<-KEGGtable.out_N;
		KEGGtable.out_S<-KEGGtable.out_S;KEGGtable.out_O<-KEGGtable.out_O;
		...
		DE_N2<-DE_N2;DE_S2<-DE_S2;DE_O2<-DE_O2;l<-l;tree<-tree;
		g1_1<-g1_1;l<-l;tree<-tree;
	}),silent=TRUE)
	platforms=NULL;
	aa=0;bb=0;cc=0;dd=0;ee=0;fff=0;gg=0;hh=0;
	try(({
		if(exists("KEGGtable.out_Affy"))aa=length(KEGGtable.out_Affy)
		if(exists("KEGGtable.out_Ag1"))bb=length(KEGGtable.out_Ag1)
		if(exists("KEGGtable.out_Ag2"))cc=length(KEGGtable.out_Ag2)
		if(exists("KEGGtable.out_Il_B"))dd=length(KEGGtable.out_Il_B)
		if(exists("KEGGtable.out_Il_L"))ee=length(KEGGtable.out_Il_L)
		if(exists("KEGGtable.out_N"))fff=length(KEGGtable.out_N)
		if(exists("KEGGtable.out_S"))gg=length(KEGGtable.out_S)
		if(exists("KEGGtable.out_O"))hh=length(KEGGtable.out_O)
		}),silent=TRUE)
	...
	mapkG_gsta_kegg_Affy=NULL;logcol_gsta_kegg_Affy=NULL;mapkG_gsta_kegg_Ag1=NULL;logcol_gsta_kegg_Ag1=NULL;mapkG_gsta_kegg_Ag2=NULL;logcol_gsta_kegg_Ag2=NULL;
	mapkG_gsta_kegg_Il_B=NULL;logcol_gsta_kegg_Il_B=NULL;mapkG_gsta_kegg_Il_L=NULL;logcol_gsta_kegg_Il_L=NULL;mapkG_gsta_kegg_N=NULL;logcol_gsta_kegg_N=NULL;
	...
	rm(graph_gsta_kegg_Affy,graph_gsta_kegg_Ag1,graph_gsta_kegg_Ag2,graph_gsta_kegg_Il_B,graph_gsta_kegg_Il_L,
	graph_gsta_kegg_N,graph_gsta_kegg_S,graph_gsta_kegg_O,
	mapkG_gsta_kegg_Affy,logcol_gsta_kegg_Affy,mapkG_gsta_kegg_Ag1,logcol_gsta_kegg_Ag1,mapkG_gsta_kegg_Ag2,logcol_gsta_kegg_Ag2,
	mapkG_gsta_kegg_Il_B,logcol_gsta_kegg_Il_B,mapkG_gsta_kegg_Il_L,logcol_gsta_kegg_Il_L,mapkG_gsta_kegg_N,logcol_gsta_kegg_N,
	...
	legend_gsta_kegg_N,legend_gsta_kegg_S,legend_gsta_kegg_O,p_v,view_ww,mapkG)
			...
			if(length(x)!=0){
				...
				if(length(which(x=="Affymetrix"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						mapkG_gsta_kegg_Affy<<-mapkG
						logcol_gsta_kegg_Affy<<-logcol
						...
						graph_gsta_kegg_Affy<<-plot(mapkG,"dot",nodeAttrs=nA)
						...
						legend_gsta_kegg_Affy<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_Affy)<<-c("Node","Node Color","logFC")
						...
						if(length(graph_gsta_kegg_Affy)!=0){
							visible(g1_1)<-FALSE
							l$Affymetrix$Graph_GSTA_KEGG<<-list()
				...
				if(length(which(x=="Agilent_OneColor"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_Ag1<<-mapkG
						logcol_gsta_kegg_Ag1<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_Ag1<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_Ag1<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_Ag1)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_Ag1,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_Ag1)!=0){
							visible(g1_1)<-FALSE
							l$Agilent_OneColor$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Agilent_TwoColor"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_Ag2<<-mapkG
						logcol_gsta_kegg_Ag2<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_Ag2<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_Ag2<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_Ag2)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_Ag2,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_Ag2)!=0){
							visible(g1_1)<-FALSE
							l$Agilent_TwoColor$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Illumina_Beadarray"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_Il_B<<-mapkG
						logcol_gsta_kegg_Il_B<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_Il_B<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_Il_B<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_Il_B)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_Il_B,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_Il_B)!=0){
							visible(g1_1)<-FALSE
							l$Illumina_Beadarray$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Illumina_Lumi"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_Il_L<<-mapkG
						logcol_gsta_kegg_Il_L<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_Il_L<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_Il_L<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_Il_L)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_Il_L,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_Il_L)!=0){
							visible(g1_1)<-FALSE
							l$Illumina_Lumi$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Nimblegen"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_N<<-mapkG
						logcol_gsta_kegg_N<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_N<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_N<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_N)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_N,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_N)!=0){
							visible(g1_1)<-FALSE
							l$Nimblegen$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Series_Matrix"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_S<<-mapkG
						logcol_gsta_kegg_S<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_S<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_S<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_S)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_S,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_S)!=0){
							visible(g1_1)<-FALSE
							l$Series_Matrix$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
				...
				if(length(which(x=="Online_Data"))!=0)
				{
					...
					gbutton("OK",border=TRUE,handler=function(h,...){
						...
						logcol[undetected]<-"white"
						names(logcol)<-names(logfcs)
						mapkG_gsta_kegg_O<<-mapkG
						logcol_gsta_kegg_O<<-logcol
						nA<-makeNodeAttrs(mapkG,fillcolor=logcol,width=10,height=1.2)
						oldpar<-par(no.readonly=TRUE)
						on.exit(par(oldpar))
						par(mar=c(3,5,0,5),mgp=c(0,0,0))
						layout(mat=matrix(c(rep(1,8),2),ncol=1,byrow=TRUE))
						graph_gsta_kegg_O<<-plot(mapkG,"dot",nodeAttrs=nA)
						image(as.matrix(seq(1,ar)),col=cols,yaxt="n",xaxt="n")
						mtext("down-regulation",side=1,at=0,line=1)
						mtext("up-regulation",side=1,at=1,line=1)
						legend<-data.frame(cbind(logcol,logfcs))
						legend_gsta_kegg_O<<-data.frame(rownames(legend),legend)
						colnames(legend_gsta_kegg_O)<<-c("Node","Node Color","logFC")
						view_ww<<-gwindow("Graph_Legend",visible=FALSE,height=480,width=650)
						gtable(legend_gsta_kegg_O,container=view_ww)
						visible(view_ww)<<-TRUE
						if(length(graph_gsta_kegg_O)!=0){
							visible(g1_1)<-FALSE
							l$Online_Data$Graph_GSTA_KEGG<<-list()
							tr<<-gtree(offspring=tree,container=g1_1)
							size(tr)<-c(300,480)
							visible(g1_1)<-TRUE
						}
