graph_gsta_kegg<-function(h,...){

	graph_gsta_kegg_params<-function(h,h2,h3,h4,h5,h6,h7,...){
		ts_name=h;ts_val=h2;ann_pk=h3;kegg_id=h4;DE=h5;children=h6;r_choice=h7;
pb_ma<-tkProgressBar(title="Graph GSTA KEGG Pathway...",label="",min=0,max=1,initial=0.1,width=500)
...
setTkProgressBar(pb_ma,0.5,title=NULL,label="")
		...
		mapkG_gsta_kegg<-mapkG
		logcol_gsta_kegg<-logcol
		...
		graph_gsta_kegg<-plot(mapkG,"dot",nodeAttrs=nA)
setTkProgressBar(pb_ma,0.8,title=NULL,label="")
		...
		legend_gsta_kegg<-legend
		colnames(legend_gsta_kegg)<-c("Node_Color","logFC")		
setTkProgressBar(pb_ma,0.9,title=NULL,label="")
		ta_legend_gsta_kegg<-tclArray()
		for(i in 0:dim(legend_gsta_kegg)[1]){ for(j in 0:dim(legend_gsta_kegg)[2]){ 
			if(i==0){ ta_legend_gsta_kegg[[i,j]]<-colnames(legend_gsta_kegg)[j] } else { 
			if(j==0){ ta_legend_gsta_kegg[[i,j]]<-rownames(legend_gsta_kegg)[i] } else {
			tem<-legend_gsta_kegg[i,j]
			ta_legend_gsta_kegg[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
			} }
		} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
		if(ts_name=="Affymetrix"){
			graph_gsta_KEGG_Affy[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_Affy[[ts_val]]<<-logcol
			legend_gsta_KEGG_Affy[[ts_val]]<<-legend_gsta_kegg
			ta_affy_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_Affy[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_Affy[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
			}
		...
		if(ts_name=="Agilent-OneColor"){
			graph_gsta_KEGG_Ag1[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_Ag1[[ts_val]]<<-logcol
			legend_gsta_KEGG_Ag1[[ts_val]]<<-legend_gsta_kegg
			ta_ag1_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_Ag1[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_Ag1[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Agilent-TwoColor"){
			graph_gsta_KEGG_Ag2[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_Ag2[[ts_val]]<<-logcol
			legend_gsta_KEGG_Ag2[[ts_val]]<<-legend_gsta_kegg
			ta_ag2_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_Ag2[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_Ag2[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Illumina-Beadarray"){
			graph_gsta_KEGG_Il_B[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_Il_B[[ts_val]]<<-logcol
			legend_gsta_KEGG_Il_B[[ts_val]]<<-legend_gsta_kegg
			ta_ilb_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_Il_B[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_Il_B[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Illumina-Lumi"){
			graph_gsta_KEGG_Il_L[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_Il_L[[ts_val]]<<-logcol
			legend_gsta_KEGG_Il_L[[ts_val]]<<-legend_gsta_kegg
			ta_ill_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_Il_L[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_Il_L[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Nimblegen"){
			graph_gsta_KEGG_N[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_N[[ts_val]]<<-logcol
			legend_gsta_KEGG_N[[ts_val]]<<-legend_gsta_kegg
			ta_nbl_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_N[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_N[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Series-Matrix"){
			graph_gsta_KEGG_S[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_S[[ts_val]]<<-logcol
			legend_gsta_KEGG_S[[ts_val]]<<-legend_gsta_kegg
			ta_smt_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_S[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_S[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
		...
		if(ts_name=="Online-Data"){
			graph_gsta_KEGG_O[[ts_val]]<<-mapkG
			nodefill_gsta_KEGG_O[[ts_val]]<<-logcol
			legend_gsta_KEGG_O[[ts_val]]<<-legend_gsta_kegg
			ta_onl_legend_gsta_kegg[[ts_val]]<<-ta_legend_gsta_kegg
			if(is.null(ma_img)==TRUE){
				try(tkpack.forget(tableData,scrX,scrY),silent=TRUE)
				try(tkwm.withdraw(w_legend),silent=TRUE)
				ma_img<<-tkrplot(getToolkitWidget(ma_frame2),function(...){plot(graph_gsta_KEGG_O[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
				ma_img2<<-ma_img
				tkpack(ma_img2)			
			}else{
				ma_img2<<-tkrreplot(ma_img,function(...){plot(graph_gsta_KEGG_O[[ts_val]],"dot",nodeAttrs=nA)},hscale=1.7,vscale=1.07)
			}
			for(i in 1:length(children)){
				x<-as.character(tcl(treeview,"item",children[i],"-values"))
				if(x==r_choice[1])p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG_Graph"))
...
"ecol","ecoi","ecoj","ecoo","ecoh","ecos","gga","ssc")
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			if(ts_name=="Affymetrix"){
				KEGG.vec<-rownames(KEGGtable.out_Affy[[ts_val]])[KEGGtable.out_Affy[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				...
				tkconfigure(r_but2,command=function(){
					...
					graph_gsta_kegg_params(ts_name,ts_val,ann_Affy[[ts_val]],kegg_id,DE_Affy_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Agilent-OneColor"){
				KEGG.vec<-rownames(KEGGtable.out_Ag1[[ts_val]])[KEGGtable.out_Ag1[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_Ag1[[ts_val]],kegg_id,DE_Ag1_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Agilent-TwoColor"){
				KEGG.vec<-rownames(KEGGtable.out_Ag2[[ts_val]])[KEGGtable.out_Ag2[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_Ag2[[ts_val]],kegg_id,DE_Ag2_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Illumina-Beadarray"){
				KEGG.vec<-rownames(KEGGtable.out_Il_B[[ts_val]])[KEGGtable.out_Il_B[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_Il_B[[ts_val]],kegg_id,DE_Il_B_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Illumina-Lumi"){
				KEGG.vec<-rownames(KEGGtable.out_Il_L[[ts_val]])[KEGGtable.out_Il_L[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_Il_L[[ts_val]],kegg_id,DE_Il_L_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Nimblegen"){
				KEGG.vec<-rownames(KEGGtable.out_N[[ts_val]])[KEGGtable.out_N[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_N[[ts_val]],kegg_id,DE_N_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Series-Matrix"){
				KEGG.vec<-rownames(KEGGtable.out_S[[ts_val]])[KEGGtable.out_S[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_S[[ts_val]],kegg_id,DE_S_2[[ts_val]],children,r_choice)
				})
			}
			if(ts_name=="Online-Data"){
				KEGG.vec<-rownames(KEGGtable.out_O[[ts_val]])[KEGGtable.out_O[[ts_val]]$'p-value'<=p_v]
				w_imp2<-tktoplevel()
				tkwm.title(w_imp2,"Select KEGG ID")
				tkwm.resizable(w_imp2,0,0)
				frame_imp2<-ttkframe(w_imp2,padding=c(3,3,12,12))
				...
				tkconfigure(r_but2,command=function(){
					tkwm.withdraw(w_imp2)
					kegg_id<-tclvalue(kids)
					graph_gsta_kegg_params(ts_name,ts_val,ann_O[[ts_val]],kegg_id,DE_O_2[[ts_val]],children,r_choice)
				})
			}
		})
	})
