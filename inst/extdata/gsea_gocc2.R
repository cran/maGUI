gsea_gocc<-function(h,...){
	...
	gsea_goCC_params<-function(h,h2,h3,h4,h5,h6,...){

		run_params<-function(h,h2,h3,h4,h5,h6,h7,h8,...){
			ts_name=h;ts_val=h2;ann_pk<-h3;db<-h4;stat_sig<-h5;p_v<-h6;children=h7;r_choice=h8;
pb_ma<-tkProgressBar(title="GSEA GO Cellular Component...",label="",min=0,max=1,initial=0.1,width=500)
...
setTkProgressBar(pb_ma,0.9,title=NULL,label="")
			...
			for(i in 0:dim(res2)[1]){ for(j in 0:dim(res2)[2]){ 
				if(i==0){ gsea_gocc[[i,j]]<-colnames(res2)[j] } else { 
				if(j==0){ gsea_gocc[[i,j]]<-rownames(res2)[i] } else {
				tem<-res2[i,j]
				gsea_gocc[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
				} }
			} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
			if(ts_name=="Affymetrix"){
				GOresultCC_Affy[[ts_val]]<<-res2
				ta_affy_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			}
			if(ts_name=="Agilent-OneColor"){
				GOresultCC_Ag1[[ts_val]]<<-res2
				ta_ag1_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Agilent-TwoColor"){
				GOresultCC_Ag2[[ts_val]]<<-res2
				ta_ag2_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Illumina-Beadarray"){
				GOresultCC_Il_B[[ts_val]]<<-res2
				ta_ilb_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Illumina-Lumi"){
				GOresultCC_Il_L[[ts_val]]<<-res2
				ta_ill_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Nimblegen"){
				GOresultCC_N[[ts_val]]<<-res2
				ta_nbl_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Series-Matrix"){
				GOresultCC_S[[ts_val]]<<-res2
				ta_smt_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
			...
			if(ts_name=="Online-Data"){
				GOresultCC_O[[ts_val]]<<-res2
				ta_onl_gsea_gocc[[ts_val]]<<-gsea_gocc
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSEA_GOCC"))}
				}
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			gsea_goCC_params(ts_name,ts_val,pre_rs,p_value_s,children,r_choice)
		})
	})
}
