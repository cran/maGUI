gsta_kegg<-function(h,...){
	...
	gsta_kegg_params<-function(h,h2,h3,h4,h5,h6,h7,...){
...
pb_ma<-tkProgressBar(title="GSTA KEGG Pathway...",label="",min=0,max=1,initial=0.1,width=500)
setTkProgressBar(pb_ma,0.2,title=NULL,label="")
			test.kegg<-gtKEGG(groups,t(stat_sig[,samp_gsta]),annotation=db,sort=TRUE)
...
setTkProgressBar(pb_ma,0.6,title=NULL,label="")
			...
			{
				x<-c()
				for(j in 1:length(subsets(test.kegg)[[rownames(res1)[i]]]))
				{
					x<-paste(subsets(test.kegg)[[rownames(res1)[i]]][j],x,sep=",")
				}
				res2$genes[i]<-x
			}
			res2<-res2[,-1]
...
setTkProgressBar(pb_ma,0.8,title=NULL,label="")
			...
			for(i in 0:dim(res2)[1]){ for(j in 0:dim(res2)[2]){ 
				if(i==0){ gsta_kegg[[i,j]]<-colnames(res2)[j] } else { 
				if(j==0){ gsta_kegg[[i,j]]<-rownames(res2)[i] } else {
				tem<-res2[i,j]
				gsta_kegg[[i,j]]<-ifelse(is.na(tem),".",ifelse(is.numeric(tem),round(tem,digits=5),as.character(tem)))
				} }
			} }
setTkProgressBar(pb_ma,1,title="Completed...",label="")	
close(pb_ma)
			if(ts_name=="Affymetrix"){
				KEGGtable.out_Affy[[ts_val]]<<-res2
				ta_affy_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			}
			if(ts_name=="Agilent-OneColor"){
				KEGGtable.out_Ag1[[ts_val]]<<-res2
				ta_ag1_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Agilent-TwoColor"){
				KEGGtable.out_Ag2[[ts_val]]<<-res2
				ta_ag2_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Illumina-Beadarray"){
				KEGGtable.out_Il_B[[ts_val]]<<-res2
				ta_ilb_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Illumina-Lumi"){
				KEGGtable.out_Il_L[[ts_val]]<<-res2
				ta_ill_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Nimblegen"){
				KEGGtable.out_N[[ts_val]]<<-res2
				ta_nbl_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Series-Matrix"){
				KEGGtable.out_S[[ts_val]]<<-res2
				ta_smt_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
			...
			if(ts_name=="Online-Data"){
				KEGGtable.out_O[[ts_val]]<<-res2
				ta_onl_gsta_kegg[[ts_val]]<<-gsta_kegg
				for(i in 1:length(children)){
					x<-as.character(tcl(treeview,"item",children[i],"-values"))
					if(x==r_choice[1]){p_enter_class<-tcl(treeview,"insert",children[i],"end",values=as.tclObj("...GSTA_KEGG"))}
				}
	...
	tkconfigure(r_but,command=function(){
		...
		tkconfigure(r_but,command=function(){
			...
			gsta_kegg_params(ts_name,ts_val,pre_rs,samp_gsta,groups,children,r_choice)
		})
	})
}
